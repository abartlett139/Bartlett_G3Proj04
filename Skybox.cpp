...
bool Skybox::generate()
{
	...
	HRESULT hRet;
	hRet = Device->CreateVertexBuffer((int)MATH::fpuMul(sizeof(D3D::EVertex), 24), 0, D3D::EVertex::FVF,
	...
	void* pVertices = NULL;
	vertexBuffer->Lock(0, (UINT)(MATH::fpuMul(sizeof(D3D::EVertex), 24)), (void**)&pVertices, 0);
	memcpy(pVertices, mesh, (MATH::fpuMul(sizeof(D3D::EVertex), 24)));
	vertexBuffer->Unlock();
	////load textures
...
void Skybox::render()
{
	...
	for (int i = 0; i < 6; ++i)
	{
		...
		Device->DrawPrimitive(D3DPT_TRIANGLESTRIP, (int)MATH::fpuMul(i, 4), 2);
	}
