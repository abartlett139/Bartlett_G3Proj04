...
Emitter::Emitter(D3D::BoundingBox* boundingBox, int numParticles)
{
	_boundingBox = *boundingBox;
	_size = 0.1f;
	_vbSize = 2048;
...
void Emitter::resetParticle(Attribute* attribute)
{
	attribute->_isAlive = true;
	...
	// no randomness for height (y-coordinate).  Snow flake
	// always starts at the top of bounding box.
	attribute->_position = _boundingBox.center();

	// particles emit randomly
	attribute->_velocity.x = D3D::GetRandomFloat(0.0f, 1.0f);
	attribute->_velocity.y = D3D::GetRandomFloat(0.0f, 1.0f);
	attribute->_velocity.z = D3D::GetRandomFloat(0.0f, 1.0f);
